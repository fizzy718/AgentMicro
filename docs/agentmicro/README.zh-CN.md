...
- assistant 向用户直接提问，或明确要求完成浏览器输入、登录、确认或授权时锁存橙色，
  直到用户继续任务；“请先不要操作”“请稍等”不会误触发。
...
- 菜单任务行左侧使用同色无边框圆角矩形；项目名按名称模式显示在标题下方，右侧固定
  ...
  绿色状态块表达，不再显示右侧绿点。
- 监听 Codex 本地全局状态中的 Desktop 未读线程集合；用户直接在 Codex App 查看完成
  任务后，AgentMicro 同步把绿色恢复为白色。来源缺失或格式异常时安全回退到本地记录，
  CLI 任务不套用 Desktop 已读推断。
- 默认名称模式为“任务标题 + 项目名”；“任务标题”模式不再重复显示项目名，
  “仅项目名”继续作为更保守的隐私选项。
- 点击 Desktop 任务时使用 `codex://threads/<session-id>` 直接切换到对应会话，
  窗口匹配与应用激活作为降级。
- 监听会话目录、已发现 rollout 和 Codex 本地全局状态，约 200ms 防抖刷新；Codex
  Desktop 运行、任务工作中或任务有独立进程时每 2 秒兜底刷新，其余时间每 5 秒刷新。
