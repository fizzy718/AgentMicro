...
| M1：Task State Engine | 下一步 | rollout 增量读取、事件配对、六态归约、当前动作 | 固定 fixtures 覆盖乱序、半行、截断和工具调用结束 |
| M2：菜单产品化 | 待开始 | V1 菜单结构、隐私设置、最近完成任务、开机启动 | 菜单验收项与隐私默认值通过 |
| M3：真实场景验证 | 待开始 | Desktop/CLI 多任务验证、性能和误报修正 | 达成 V1 全部退出条件 |

M0 的 `active/idle` 仅用于打通发现、展示和返回任务的端到端链路，不会作为最终 V1 状态对外承诺。
