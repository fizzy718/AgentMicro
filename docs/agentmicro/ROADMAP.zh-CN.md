...
| M3：真实场景验证 | 下一步 | Desktop/CLI 多任务验证、性能和误报修正 | 达成 V1 全部退出条件 |

M1 已替换 M0 的 `active/idle` 展示。M2 将状态能力包装成可直接启动的菜单栏产品；自动化已对设置窗口做真实视觉检查，但当前 macOS UI 自动化接口不暴露隐藏菜单栏项，菜单项本身由模型测试和打包后进程烟测覆盖。没有可靠 owner 的近期 Desktop rollout 保持 `unknown`，不通过 app-server PID 猜测任务归属；该关联能力必须在 M3 用真实 Desktop 多任务场景验证。
