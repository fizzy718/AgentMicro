...
The menu shows the most recently changed tasks, with working tasks first. Each row contains a state block, task title, project, current-turn duration, and an optional fast-mode lightning badge. In direct-download builds, the second line also shows a smoothed CPU level for a CLI task's complete process tree; Desktop rows say `CPU shared` because Codex does not expose reliable per-conversation process ownership. A search button in the header finds observed tasks by project name, task title, or bounded recent local user/assistant conversation text, then opens the matching Codex conversation without showing chat snippets. System, developer, tool, and reasoning content is excluded; short queries search metadata only. The native search field preserves focus and results through background refreshes, waits for macOS input methods to commit composed text, and ranks exact title/project matches ahead of conversation-only hits. A compact weekly Codex quota card sits below the task rows and shows consumed percentage, reset timing, pace variance, projected exhaustion, and quota markers; the bar itself shows the exact remaining percentage on the same axis as its markers. The menu bar icon mirrors the first six rows as a 2×3 grid of horizontal rounded rectangles. Slots follow task order left to right across the top and right to left across the bottom; thinking, unread, needs-input, and error blocks use a smooth 20-frame-per-second breathing sequence while idle blocks remain static.
...
Settings use a CodexBar-style sidebar with Guide, General, Task Display, and Updates pages. Direct-download builds include a default-on task CPU switch; sampling occurs every two seconds only while the menu is open and stops immediately when disabled or closed. The Guide is opened automatically on first launch, explains all five colors, and includes a checked-by-default “Don’t show this guide on next launch” checkbox. All AgentMicro UI copy is available in the same 23 interface languages exposed by CodexBar.

AgentMicro reads known local Codex process and session metadata. Current-date directory discovery is supplemented by recently updated, unarchived rollout paths from Codex's local thread database, so resuming an old conversation is detected without scanning every historical directory. Search reads bounded recent user/assistant text from those already observed rollouts into a transient memory cache; it excludes tool output and never displays matched chat text. Optional Enhanced Status Detection can use macOS Accessibility, only after explicit opt-in, to recognize the uniquely selected Codex task and visible approval/error controls. Base mode never requires that permission, and the enhanced reader does not click or type. AgentMicro does not upload task titles, prompts, responses, source code, command output, or session files.
